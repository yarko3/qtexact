 ...
 */
class MDTreeNode<V> extends RootedTreeNode<V> {
	 ...
	 */
	protected MDTreeNode(MDTreeNode<V> copy) {
		super();
		type = copy.type;
	...
	protected void setCompNumForSubtree(int compNumber) {
		...
		MDTreeNode<V> currentChild = (MDTreeNode<V>) getFirstChild();
		while (currentChild != null) {
			currentChild.setCompNumForSubtree(compNumber);
			currentChild = (MDTreeNode<V>) currentChild.getRightSibling();
		}
	}
	...
	protected void markAncestorsBySplit(SplitDirection splitType) {
		...
		MDTreeNode<V> parent = (MDTreeNode<V>) getParent();		
	...
	protected void promote(SplitDirection splitType) {
		...
		MDTreeNode<V> toPromote = (MDTreeNode<V>) getFirstChild();		
		
		// Promote each child marked by the given type.
		while (toPromote != null) {			
			
			MDTreeNode<V> nextToPromote = (MDTreeNode<V>) toPromote.getRightSibling();
	...
	protected Collection<MDTreeLeafNode<V>> getAlpha() {		
		...
		LinkedList<MDTreeLeafNode<V>> alpha = new LinkedList<MDTreeLeafNode<V>>();
		
		Iterator<RootedTreeNode<V>> leafIt = getLeaves().iterator();
		while (leafIt.hasNext()) {
			alpha.addAll(((MDTreeLeafNode<V>) leafIt.next()).getAlpha());			
		}
	...
	protected void removeDegenerateDuplicatesFromSubtree() {
		...
		while(currentChild != null) {
			...
			MDTreeNode<V> nextChild = (MDTreeNode<V>) currentChild.getRightSibling();
	...
	protected void clearVisited() {
		...
		Iterator<RootedTreeNode<V>> vertexIt = getLeaves().iterator();		
		while (vertexIt.hasNext()) {
			((MDTreeLeafNode<V>)vertexIt.next()).clearVisited();
		}		
	}
