 ...
 */
class RootedTreeNode<V> {

	// The parent of this node.
	private RootedTreeNode<V> parent;
	
	// The first child of this node.
	private RootedTreeNode<V> firstChild;
	
	// This node's sibling to its left.
	private RootedTreeNode<V> leftSibling;
	
	// This node's sibling to its right.
	private RootedTreeNode<V> rightSibling;
	 ...
	 */ 
	protected void addChild(RootedTreeNode<V> child) {
		child.remove();
		if (firstChild != null) {
	 ...
	 */
	protected void replaceWith(RootedTreeNode<V> replacement) {
		replacement.remove();
	 ...
	 */
	protected void insertBefore(RootedTreeNode<V> justBefore) {
		remove();
	 ...
	 */
	protected void insertAfter(RootedTreeNode<V> justAfter) {
		remove();
	...
	protected void makeFirstChild() {
		...
		RootedTreeNode<V> newRightSibling = parent.firstChild;
		remove();
	...
	protected RootedTreeNode<V> getParent() {
		return parent;
	}


	/* Returns the first child of this node. */
	protected RootedTreeNode<V> getFirstChild() {
		return firstChild;
	}
	

	/* Returns the left sibling of this node. */
	protected RootedTreeNode<V> getLeftSibling() {
		return leftSibling;
	}


	/* Returns the right sibling of this node. */
	protected RootedTreeNode<V> getRightSibling() {
		return rightSibling;
	}

	
	/* Returns a collection of the leaves of the subtree rooted at this node. */
	protected Collection<RootedTreeNode<V>> getLeaves() {
		
		LinkedList<RootedTreeNode<V>> leaves = new LinkedList<RootedTreeNode<V>>();
		
		if (isALeaf()) { leaves.add(this); }
		else {
			RootedTreeNode<V> currentChild = firstChild;
	 ...
	 */
	protected void addChildrenFrom(RootedTreeNode<V> from) {
		RootedTreeNode<V> currentChild = from.firstChild;
		while (currentChild != null) {
			RootedTreeNode<V> nextChild = currentChild.rightSibling;
	...
	protected void replaceThisByItsChildren() {
		...
		RootedTreeNode<V> currentChild = getFirstChild();
		while (currentChild != null) {
			RootedTreeNode<V> nextChild = currentChild.getRightSibling();
	 ...
	 */
	protected void replaceChildrenWith(RootedTreeNode<V> replacement) {
		RootedTreeNode<V> currentChild = getFirstChild();
		while (currentChild != null) {
			RootedTreeNode<V> nextChild = currentChild.getRightSibling();
			currentChild.remove();
