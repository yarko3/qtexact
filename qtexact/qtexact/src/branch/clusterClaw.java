...
public class clusterClaw<V> extends clusterBranch<V> 
{

	public clusterClaw(Controller<V> controller) {
		super(controller);
	...
	public branchingReturnC<V> branchingRules(branchingReturnC<V> s, SearchResult<V> searchResult)
	{
		...
		//check if claw is present
		if (certificate.getFlag() == -14)
		{
			...
			if (!s.getChanges().contains(new myEdge<V>(new Pair<V>(lexResult.get(0), lexResult.get(2)), false, directed))
				...
				&& !s.getChanges().contains(new myEdge<V>(new Pair<V>(lexResult.get(2), lexResult.get(3)), false, directed)))
			{
				if (output)
				{
				...
				addResult(s, lexResult.get(2), lexResult.get(3));
				controller.branch(s);
				revert(s, 3);
				if (output)
				{
				...
				&& !s.getChanges().contains(new myEdge<V>(new Pair<V>(lexResult.get(2), lexResult.get(1)), true, directed)))
			{
				...
				deleteResult(s, lexResult.get(2), lexResult.get(1));
				controller.branch(s);
				revert(s, 2);
				if (output)
				{
			...
			if (!s.getChanges().contains(new myEdge<V>(new Pair<V>(lexResult.get(0), lexResult.get(2)), false, directed))
				&& !s.getChanges().contains(new myEdge<V>(new Pair<V>(lexResult.get(1), lexResult.get(3)), true, directed)))
			{
				if (output)
				{
					//change progress percent
				...
				addResult(s, lexResult.get(0), lexResult.get(2));
				deleteResult(s, lexResult.get(1), lexResult.get(3));
				controller.branch(s);
				revert(s, 2);
				if (output)
				{
